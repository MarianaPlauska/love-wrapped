...
export const WheelSlide = ({ data, palette }: WheelSlideProps) => {
  ...
  return (
    <SlideSurface palette={palette} className="justify-between">
      ...
      <SlideScrollBody className="flex flex-col items-center px-6 py-4">
        <div className="relative">
          ...
          <motion.div
            ...
            transition={{ duration: 3.2, ease: [0.15, 0, 0.15, 1] }}
            className="relative h-56 w-56 overflow-hidden rounded-full border-4 border-white/15 shadow-2xl sm:h-64 sm:w-64"
          ...
          >
            {data.items.map((item, index) => {
              ...
              return (
                <div
                  key={index}
                  className="absolute left-1/2 top-1/2 w-16 origin-left"
                  style={{
                    transform: `translateY(-50%) rotate(${angle + sliceAngle / 2}deg) translateX(26px)`,
                  }}
                >
                  <span
                    className="block rotate-180 text-center font-black uppercase tracking-wider text-zinc-950 drop-shadow-sm"
                    style={{
                      writingMode: 'vertical-rl',
                      fontSize: label.length > 10 ? '0.5rem' : label.length > 6 ? '0.58rem' : '0.68rem',
                      lineHeight: 1,
                      maxHeight: '4.5rem',
                      overflow: 'hidden',
                    }}
          ...
          <button
            ...
            onClick={spinWheel}
            disabled={isSpinning}
            className="absolute left-1/2 top-1/2 z-10 flex h-14 w-14 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-zinc-950 text-xs font-black uppercase tracking-wider text-lime-300 shadow-xl ring-4 ring-white/20 active:scale-95 disabled:opacity-70 sm:h-16 sm:w-16"
          >
            {isSpinning ? '...' : 'Girar'}
          </button>
        </div>

        <div className="mt-6 w-full max-w-[280px] rounded-2xl bg-white/8 p-4 text-center backdrop-blur-sm">
          {selectedIndex !== null ? (
            <motion.div
